...
- [ ] Landing page e documentação passam a comunicar o recurso
...
| Disponibilidade por plano | Disponível em todos os planos, inclusive Free | É a prova de ROI que converte Free → pago; travar no pago esconderia justamente o argumento de venda | n (default do agente) |
...
6. WHEN o lojista está no painel THEN o item "Pedidos" SHALL aparecer na navegação do Sidebar (desktop) e do MobileTabBar (mobile), marcado como ativo em `/painel/pedidos`.
...
3. WHEN o visitante compara os planos THEN "Histórico de pedidos" SHALL constar como incluso nos três planos.
...
| ORD-24 | P1: Segurança (sem GRANT/policy para `anon`; RLS por dono; service role server-only) | Design | Pending |
...
**Coverage:** 26 total, 0 mapeados para tasks (fase Tasks pendente), 0 sem mapeamento ⚠️
...
- [ ] Lojista consegue responder "quantos pedidos e quanto vendi esse mês?" em menos de 5 segundos após abrir o painel
